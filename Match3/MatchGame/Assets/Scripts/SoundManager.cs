...
public class SoundManager : Singleton<SoundManager>
{
    ...
    public AudioSource PlayClipAtPoint(AudioClip clip, Vector3 position, float volume = 1f, bool randomizedPitch = true)
    {
        if (clip != null)
        {
    ...
    public AudioSource PlayRandom(AudioClip[] clips, Vector3 position, float volume = 1f, bool isLooping = false)
    {
        if (clips != null)
        {
            if (clips.Length != 0)
    ...
    {
        PlayRandom(loseClips, Vector3.zero, fxVolume * 0.25f);
    }
